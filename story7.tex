...
$\frac{2 \times e} {3} =e-n+2$, resulting in $\frac{e}{3}=n-2$ or $e=3\times n -6$. This $3 \times n-6$ is the maximum number of edges a planar graph can have (He derived Equation \ref{eqn:maxplanar} once again) . Ajur reasoned that is all vertices have degree 6 or more, then $e~\ge~ 3 \times n$, contradicting that the graph is planar.
