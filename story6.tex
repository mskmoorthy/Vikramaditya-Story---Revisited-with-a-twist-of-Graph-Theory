...
Rishnak smiled and said, ``Good question. Two graphs are called \textit{isomorphic}---meaning structurally the same---if they are equivalent under a vertex relabeling \index{isomorphism}. For example, if in the third graph [Figure~\ref{8g2}], vertex~2 is relabeled as vertex~3 and similarly vertex~3 is relabeled as vertex~2, then the third graph becomes equivalent or isomorphic to the other two graphs.''
...
Rishnak was pleased that Ajur was thinking about some of the practical applications of graph theory. Rishnak said, ``There is an easier method to test for isomorphism between two rooted trees. Let's start with how we can encode a labeled tree. We'll use an approach called a Pr\"ufer code. \index{Pr\"ufer code} The construction is iterative, and the Pr\"ufer code of a labeled tree with~$n$ vertices is a code of length~$n-2$. Here's the algorithm, but first, remember that a leaf of a tree is a vertex that is connected to only one other vertex.
...
Ajur said, ``A subgraph of~$G$. I understand. This approach could be used to test whether graph~$G$ with~$n$ vertices has a Hamiltonian cycle by choosing graph~$H$ to be a cycle of length~$n$ and asking whether there is a subgraph of~$G$ isomorphic to~$H$.'' \index{subgraph}
