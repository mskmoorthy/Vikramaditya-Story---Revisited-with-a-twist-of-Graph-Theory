...
Hamiltonian path. So he drew the graph in Figure \ref{5g4}. Rishnak mentioned to Ajur that this Hamiltonian path is not unique and there are other Hamiltonian paths.\footnote{Find a Hamiltonian path distinct from the one shown here.}
...
\label{5t1}
\end{table}
Rishnak asked Ajur to find the message that is encoded here in Table \ref{5t2}.
\begin{table}
\centering
...
f& v& d &i& e& u\\
\end{tabular}
\caption{What messafe is encoded here}
\label{5t2}
\end{table}
...
\begin{enumerate}
    \item First one shows that the graph is connected.
...
Ajur was able to follow along and loved the subtle and the clever arguments used in the proof. 
 ...
 \textbf{Question for the fifth day:} Is there a Hamiltonian path in a complete bipartite graph $K_{2,4}$ i.e., Vertices 1 and 2 are adjacent to all the vertices 3, 4, 5 and 6 and there are no other edges. 
 
 \textbf{Answer:} Ajur drew the graph in Figure \ref {5q1}. In a Hamiltonian path vertices have to alternate between the vertices in different partitions. If we start with a vertex in the larger set say B, then the Hamiltonian path has to be B, A, B, A, B, A, B - This implies that there are three vertices in A partition; but, in reality, there are only two vertices. Hence there cannot be a Hamiltonian path in $K_{2,4}$.
...
\begin{tikzpicture}
  ...
  \node (n6) at (14, 2) {B};
   ...
   \foreach \from/\to in {n1/n3,n1/n4,n1/n5,n2/n3,n2/n4,n2/n5,n1/n6,n2/n6}
    \draw (\from) -- (\to);
    \end{tikzpicture}
\caption{ A complete  Bipartite Graph $K_{2,4}$. Does it have a Hamiltonian path?}\label{5q1}
\end{center}
\end{figure}
