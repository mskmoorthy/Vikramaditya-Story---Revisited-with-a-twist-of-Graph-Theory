...
Ajur was able to follow along and loved the subtle and the clever arguments used in the proof. 
 ...
 \textbf{Question for the fifth day:} Is there a Hamiltonian path in a complete bipartite graph $K_{2,4}$ i.e., Vertices 1 and 2 are adjacent to all the vertices 3, 4, 5 and 6 and there are no other edge. 
 
 \textbf{Answer:} Ajur drew the following graph Figure \ref {5q1}. In a Hamiltonian Path vertices have to alternate between the vertices in different partitions. If we start with a vertex in the larger set say B, then the Hamiltonian Path has to be B, A, B, A, B, A, B - This implies that there are three vertices in A partition - But there are only two vertices. Hence there cannot be a Hamiltonian Path in $K_{2,4}$.
